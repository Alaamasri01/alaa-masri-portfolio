...
(function () {
  ...
  S.shelfSection = function (w, h) {
    ...
    s += D.dimH([x0 + 20, x0 + 31.2, x0 + 91.8], 157, 151, ['56', '300']);
    s += D.dimH([x0 + 82.2, x0 + 85.8, x0 + 91.8], 34, 40, ['17', '30']);
    s += D.dimV([40, 45, 110], x0 + 98, x0 + 92.5, ['25', '325 typ.']);
    // keynotes
    s += D.key(x0 + 10, 64, 56, 58, '1') + D.key(x0 + 21.2, 66, 56, 66, '2') + D.key(x0 + 24.9, 77, 56, 74, '3') + D.key(x0 + 29.2, 86, 56, 82, '4') + D.key(x0 + 31.2, 94, 56, 90, '5');
    s += D.key(x0 + 60, 42.5, 72, 28, '7') + D.key(x0 + 88, 42.4, x0 + 98, 26, '6') + D.key(x0 + 84, 45, x0 + 100, 56, '8') + D.key(x0 + 25, 80, 56, 98, '9') + D.key(x0 + 30, 20.5, 52, 30, '10');
    return D.svg(w, h, s);
  };
  ...
  S.revealPlan = function (w, h) {
    ...
    s += lab(53, y0 + 21, 4, h - 1.2, 'Batten 25 × 50') + lab(100, y0 + 30, 64, h - 1.2, 'Plaster on blockwork');
    return D.svg(w, h, s);
  };
