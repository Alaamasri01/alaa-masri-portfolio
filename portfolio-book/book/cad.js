...
(function () {
  ...
  var FS = 2.5; // 7.1 pt — body text of drawings; smallest labels are 2.45 mm (6.9 pt)
  ...
  var P = {
    ...
    // Hatches are drawn as real clipped vector lines (SVG <pattern> fills get rasterised by the PDF engine).
    fill: function (name) { return 'H:' + name; },
    _clip: 0,
    hatch: function (name, x, y, w, h, clipShape) {
      var id = 'hc' + (P._id || '') + (P._clip++), s = '', c = C.mid, i;
      if (name === 'tim') for (i = y + .6; i < y + h; i += 1.2) s += '<line x1="' + x + '" y1="' + i.toFixed(2) + '" x2="' + (x + w) + '" y2="' + i.toFixed(2) + '"/>';
      else if (name === 'timv') for (i = x + .6; i < x + w; i += 1.2) s += '<line x1="' + i.toFixed(2) + '" y1="' + y + '" x2="' + i.toFixed(2) + '" y2="' + (y + h) + '"/>';
      else if (name === 'st' || name === 'cc') {
        var st = name === 'st' ? 2.26 : 3.39;
        for (i = -h; i < w; i += st) s += '<line x1="' + (x + i).toFixed(2) + '" y1="' + (y + h) + '" x2="' + (x + i + h).toFixed(2) + '" y2="' + y + '"/>';
        if (name === 'cc') for (i = 0; i < w + h; i += st) s += '<line x1="' + (x + i).toFixed(2) + '" y1="' + (y + h) + '" x2="' + (x + i - h).toFixed(2) + '" y2="' + y + '"/>';
      } else if (name === 'up') {
        for (var r = y; r < y + h + 2; r += 2) { var d = 'M' + x + ' ' + (r + 2); for (var k = x; k < x + w; k += 2) d += ' Q' + (k + 1) + ' ' + r + ' ' + (k + 2) + ' ' + (r + 2); s += '<path d="' + d + '" fill="none"/>'; }
      } else if (name === 'dot') {
        var dd = '';
        for (var yy = y + .7; yy < y + h; yy += 1.4) for (var xx = x + .7; xx < x + w; xx += 1.4) dd += 'M' + (xx - .13).toFixed(2) + ' ' + yy.toFixed(2) + 'a.13 .13 0 1 0 .26 0a.13 .13 0 1 0 -.26 0';
        return '<clipPath id="' + id + '">' + clipShape + '</clipPath><path clip-path="url(#' + id + ')" d="' + dd + '" fill="' + c + '" stroke="none"/>';
      }
      var sw = { tim: .08, timv: .08, st: .1, cc: .09, up: .09 }[name] || .08;
      return '<clipPath id="' + id + '">' + clipShape + '</clipPath><g clip-path="url(#' + id + ')" stroke="' + c + '" stroke-width="' + sw + '">' + s + '</g>';
    },
    ...
    rect: function (x, y, w, h, o) {
      o = o || {}; var hp = '', f = o.fill || 'none';
      if (f.indexOf('H:') === 0) { hp = P.hatch(f.slice(2), x, y, w, h, '<rect x="' + x + '" y="' + y + '" width="' + w + '" height="' + h + '"/>'); f = 'none'; }
      return hp + '<rect' + a({ x: x, y: y, width: w, height: h, fill: f, stroke: o.stroke === undefined ? C.ink : o.stroke, 'stroke-width': o.w || W.med, 'stroke-dasharray': o.dash, rx: o.rx }) + '/>';
    },
    path: function (d, o) { o = o || {}; var hp = '', f = o.fill || 'none'; if (f.indexOf('H:') === 0) { var b = o.bbox; hp = P.hatch(f.slice(2), b[0], b[1], b[2], b[3], '<path d="' + d + '"/>'); f = 'none'; } return hp + '<path' + a({ d: d, fill: f, stroke: o.stroke === undefined ? C.ink : o.stroke, 'stroke-width': o.w || W.med, 'stroke-dasharray': o.dash, 'marker-end': o.arrow ? 'url(#' + P._id + 'ar)' : undefined }) + '/>'; },
    ...
    bubble: function (x, y, t, r, o) { o = o || {}; r = r || 2.4; return P.circle(x, y, r, { w: W.thin, stroke: o.c || C.ink, fill: o.fill || C.paper }) + P.text(x, y + .75, t, { anchor: 'middle', size: o.size || 2.4, weight: 500, fill: o.c || C.ink }); },
    // Numbered leader: dot at target, line to a bronze numbered bubble.
    tag: function (x, y, tx, ty, n) { return P.circle(x, y, .45, { fill: C.bronze, stroke: 'none' }) + P.line(x, y, tx, ty, W.thin, C.bronze) + P.circle(tx, ty, 2.45, { fill: C.paper, stroke: C.bronze, w: W.thin }) + P.text(tx, ty + .85, n, { anchor: 'middle', size: 2.45, weight: 600, fill: C.bronze }); },
    label: function (x, y, tx, ty, t, anchor) { return P.circle(x, y, .4, { fill: C.bronze, stroke: 'none' }) + P.line(x, y, tx, ty, W.thin, C.bronze) + P.line(tx, ty, tx + (anchor === 'end' ? -3 : 3), ty, W.thin, C.bronze) + P.text(tx + (anchor === 'end' ? -3.6 : 3.6), ty + .7, t, { anchor: anchor || 'start', size: 2.5 }); },
    ...
    marker: function (x, y, t, ang) {
      ...
      return '<path d="M' + l + ' ' + lt + ' L' + px + ' ' + py + ' L' + rr + ' ' + rt + ' Z" fill="' + C.ink + '"/>' + P.circle(x, y, r, { fill: C.paper, w: W.med }) + P.text(x, y + .8, t, { anchor: 'middle', size: 2.5, weight: 600 });
    },
    statusTag: function (x, y, t, anchor) {
      var w = t.length * 1.78 + 4.4, x0 = anchor === 'end' ? x - w : x;
      return P.rect(x0, y - 3.8, w, 5.4, { stroke: C.bronze, w: W.thin }) + P.caps(x0 + 2.2, y + .2, t, { fill: C.bronze, size: 2.45, ls: .3 });
    },
    ...
    titleBlock: function (x, y, w, o) {
      ...
      if (w < 130) { // stacked block for narrow drawings
        var refT = o.ref + ' · ' + (o.scale || 'NTS'), fits = (o.status.length + refT.length) * 1.72 + 8 < w, bh = fits ? 17.5 : 23;
        y = y + 10 - 0.5 - bh;
        s = P.rect(x, y, w, bh, { w: W.thin }) + P.line(x, y + 10.5, x + w, y + 10.5, W.hair);
        s += P.caps(x + 1.6, y + 3.7, kick, { size: 2.3, fill: C.mid, ls: .18 }) + P.text(x + 1.6, y + 8.4, o.title, { size: 3.3, serif: true });
        s += P.caps(x + 1.6, y + 15.2, o.status, { size: 2.3, fill: C.bronze, weight: 600, ls: .16 });
        if (fits) s += P.caps(x + w - 1.6, y + 15.2, refT, { size: 2.3, weight: 500, ls: .16, anchor: 'end' });
        else s += P.line(x, y + 17.4, x + w, y + 17.4, W.hair) + P.caps(x + 1.6, y + 21.2, refT, { size: 2.3, weight: 500, ls: .16 });
        return s;
      }
      y -= 1; var h = 10.5, refTxt = o.ref + ' · ' + (o.scale || 'NTS'), c2 = x + w - Math.max(24, refTxt.length * 1.72 + 5), c1 = c2 - Math.max(22, o.status.length * 1.72 + 5);
      s = P.rect(x, y, w, h, { w: W.thin }) + P.line(c1, y, c1, y + h, W.hair) + P.line(c2, y, c2, y + h, W.hair);
      s += P.caps(x + 1.6, y + 3.7, kick, { size: 2.3, fill: C.mid, ls: .2 });
      s += P.text(x + 1.6, y + 8.5, o.title, { size: 3.3, serif: true });
      s += P.caps(c1 + 1.6, y + 3.7, 'Status', { size: 2.3, fill: C.mid });
      s += P.caps(c1 + 1.6, y + 8.2, o.status, { size: 2.3, fill: C.bronze, weight: 600, ls: .16 });
      s += P.caps(c2 + 1.6, y + 3.7, 'Ref · scale', { size: 2.3, fill: C.mid });
      s += P.caps(c2 + 1.6, y + 8.2, o.ref + ' · ' + (o.scale || 'NTS'), { size: 2.3, weight: 500, ls: .16 });
      return s;
    },
  ...
  P.walnutPlan = function (w, h) {
    ...
    // zone names
    s += P.caps(X + 20, Y + 57, 'Sleep zone', { size: 2.45, fill: C.bronze }) + P.caps(X + 6, Y + RH - 12.5, 'Media & display wall', { size: 2.45, fill: C.bronze }) + P.caps(X + RW - 40, dy - 3, 'Circulation', { size: 2.45, fill: C.bronze });
  ...
  P.wardrobeModule = function (w, h, dark) {
    P.theme(dark);
    var x0 = 555, x1 = 845, y0 = 40, y1 = 940, pad = 6, top = 6, dh = h - 37;
  ...
  P.conventions = function (w, h) {
    ...
    s += P.marker(col * 2 + 4, 10, 'E1', 0) + P.text(col * 2 + 11, 10.8, 'Elevation / section', { size: 2.5 });
    s += P.bubble(col * 2 + 4, 19, 'A', 2.2) + P.text(col * 2 + 11, 19.8, 'Grid reference', { size: 2.5 });
    s += P.circle(col * 2 + 4, 27, 2.1, { fill: C.paper, stroke: C.bronze }) + P.text(col * 2 + 4, 27.8, '3', { anchor: 'middle', size: 2.45, weight: 600, fill: C.bronze }) + P.text(col * 2 + 11, 27.8, 'Finish / note key', { size: 2.5 });
